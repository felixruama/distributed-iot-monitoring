...
--
-- Host: mysql
-- Tempo de geração: 11/05/2026 às 16:27
...
CREATE DEFINER=`root`@`%` PROCEDURE `SP_CriarUtilizador` (IN `p_Nome` VARCHAR(100), IN `p_Email` VARCHAR(50), IN `p_Password` VARCHAR(255), IN `p_Tipo` VARCHAR(50), IN `p_Telemovel` VARCHAR(12), IN `p_DataNascimento` DATE, IN `p_Equipa` INT)   BEGIN
    ...
    ELSE
        ...
        -- Criar utilizador no motor do MySQL
        ...
        SET @sql_grant = CONCAT('GRANT SELECT, INSERT, UPDATE ON labirinto_DB.* TO ''', p_Email, '''@''%'';');
        PREPARE stmt_grant FROM @sql_grant; EXECUTE stmt_grant; DEALLOCATE PREPARE stmt_grant;

        SELECT LAST_INSERT_ID() AS IDUtilizador, 'Utilizador criado com sucesso!' AS Mensagem;
    END IF;
END$$
...
--
-- Estrutura para tabela `historico_medicoespassagens`
--
...
--
-- Estrutura para tabela `historico_ocupacaolabirinto`
--
...
--
-- Estrutura para tabela `historico_simulacao`
--
...
--
-- Estrutura para tabela `medicoespassagens`
--
...
--
-- Estrutura para tabela `ocupacaolabirinto`
--
...
--
-- Despejando dados para a tabela `simulacao`
--
...
--
-- Restrições para tabelas `corredor`
--
...
--
-- Restrições para tabelas `marsami`
--
ALTER TABLE `marsami`
...
--
-- Restrições para tabelas `medicoespassagens`
--
...
--
-- Restrições para tabelas `ocupacaolabirinto`
--
