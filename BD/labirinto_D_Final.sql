...
--
-- Host: db_pc2
-- Tempo de geração: 10-Maio-2026 às 23:17
...
--
-- Base de dados: `labirinto_DBNEW.sql`
--
...
CREATE DEFINER=`root`@`%` PROCEDURE `SP_IniciarSimulacao` (IN `p_IDSimulacao` INT)   BEGIN
    ...
    -- =========================================================================
    -- O "AIRBAG": Se houver QUALQUER erro a meio (incluindo os nossos SIGNALs),
    -- o MySQL cancela todas as cópias e limpezas que fez até agora!
    -- =========================================================================
    DECLARE EXIT HANDLER FOR SQLEXCEPTION 
    BEGIN
        ROLLBACK;
        RESIGNAL; -- Mostra o erro no ecrã para saberes o que falhou
    END;

    -- Inicia o modo "Tudo ou Nada"
    START TRANSACTION;

    -- =========================================================================
    -- PASSO A: BARREIRA DE SEGURANÇA
    -- =========================================================================
    ...
    -- =========================================================================
    -- PASSO B: EXPORTAR DADOS
    -- =========================================================================
    SELECT COUNT(*) INTO v_simulacoes_terminadas FROM simulacao WHERE Estado = '2';
    
    IF v_simulacoes_terminadas > 0 THEN
        
        INSERT INTO historico_simulacao (IDSimulacao, Descricao, Equipa, DataHoraInicio, DataHoraFim, SimulacaoIniciada, Pontos, Criador, TempMaxAlerta, TempMinAlerta, RuidoMaxAlerta, SegundosIntervaloAlertas, Periodicidade, Estado)
        SELECT IDSimulacao, Descricao, Equipa, DataHoraInicio, DataHoraFim, SimulacaoIniciada, Pontos, Criador, TempMaxAlerta, TempMinAlerta, RuidoMaxAlerta, SegundosIntervaloAlertas, Periodicidade, Estado
        ...
        -- =========================================================================
        -- PASSO C: LIMPEZA DAS TABELAS "VIVAS"
        -- Trocámos o TRUNCATE por DELETE para não quebrar a Transação!
        -- =========================================================================
        DELETE FROM som;
    ...
    -- =========================================================================
    -- PASSO D: INICIAR A NOVA SIMULAÇÃO COM CAMINHO LIVRE
    -- =========================================================================
    UPDATE simulacao 
    SET Estado = '1', DataHoraInicio = CURRENT_TIMESTAMP, SimulacaoIniciada = 1 
    WHERE IDSimulacao = p_IDSimulacao;
    
    -- Se a simulação não for encontrada, disparamos o erro. O "Airbag" lá em cima
    -- vai apanhar este erro, desfazer as cópias do Passo B e a limpeza do Passo C!
    IF ROW_COUNT() = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Erro: Simulação não encontrada.';
    END IF;

    -- Se chegou até aqui sem bater em nenhum erro, guarda tudo definitivamente!
    COMMIT;
...
--
-- Índices para tabela `historico_medicoespassagens`
--
...
--
-- Índices para tabela `historico_ocupacaolabirinto`
--
...
--
-- Índices para tabela `historico_simulacao`
--
